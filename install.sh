...
check_token() {
  ...
  if [ "$USER_TOKEN" = "zero" ]; then
    ...
    echo -e "${GREEN}Token Salah! Beli Kode Token Di zerodev${NC}"
    echo -e "${YELLOW}TELEGRAM : @ext4you${NC}"
    echo -e "${YELLOW}WHATSAPP : +6281997715124${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 15K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© ZERO-DEV${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/zerodevxc/Autoinstall-/raw/main/stellar.zip")        
        break
        ;;
      2)
        THEME_URL=$(echo -e "https://github.com/zerodevxc/Autoinstall-/raw/main/billing.zip")
        break
        ;;
      3)
        THEME_URL=$(echo -e "https://github.com/zerodevxc/Autoinstall-/raw/main/enigma.zip")
        break
        ;; 
      x)
        return
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer zerodev   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : t.me/ext4you    ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : zerodev  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by zerodev  ${NC}"
